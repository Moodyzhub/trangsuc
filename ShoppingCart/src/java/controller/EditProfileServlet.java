...
public class EditProfileServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            udao.updateUsers(email, country, contact, name);

            request.setAttribute("msg", "Update successfully");

            // refesh user session
            HttpSession session = request.getSession();
            User utf8 = (User) session.getAttribute("auth"); // lay lai user session cu
            User u = udao.userLogin(utf8.getEmail(), utf8.getPassword()); // lay thang moi
