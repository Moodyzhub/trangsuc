 ...
 * @author admin
 */
@WebServlet(name = "UpdateCate", urlPatterns = {"/update-cate"})
public class UpdateCate extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet UpdateCate</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet UpdateCate at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int cid;
        try {
            cid = Integer.parseInt(id_row);
            CategoryDAO dAO = new CategoryDAO();
            Category c = dAO.getCategoryById(cid);
            request.setAttribute("cute", c);
